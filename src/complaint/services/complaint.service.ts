...
import { Complaint, ComplaintDocument, ComplaintStatus, ComplaintTarget } from '../schemas/complaint.schema';
import {
    CreateComplaintDto,
    UpdateComplaintDto,
    AdminResponseDto,
...
export class ComplaintService {
    ...
    // ─── Find by Client ─────────────────────────────────────────────────────
    async findByClient(userId: string, options: {
        status?: ComplaintStatus;
        page: number;
        limit: number;
    }) {
    ...
    async update(
        ...
        if (complaint.status !== ComplaintStatus.OPEN) {
            throw new ForbiddenException('Cannot update complaint after it has been processed');
        }
    ...
    async addClientFeedback(
        ...
        if (complaint.userId.toString() !== userId) {
            throw new ForbiddenException('You can only feedback on your own complaints');
        }
    ...
    async delete(id: string, userId: string): Promise<void> {
        ...
        if (complaint.status !== ComplaintStatus.OPEN) {
            throw new ForbiddenException('Cannot delete complaint after it has been processed');
        }
