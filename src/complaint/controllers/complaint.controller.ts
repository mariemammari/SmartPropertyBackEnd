...
export class ComplaintController {
    ...
    async findAll(
        ...
        if (userRole === UserRole.BRANCH_MANAGER) {
            if (!userBranchId) {
                throw new ForbiddenException('Branch manager not assigned to any branch');
            }
    ...
    async getById(@Param('id') id: string, @Request() req: any) {
        ...
        const isOwner = possibleUserIds.some(uid => idsMatch(complaint.userId, uid));
    ...
    async addAdminResponse(
        ...
        if (userRole === UserRole.BRANCH_MANAGER) {
            ...
            if (complaint.branchId?.toString() !== userBranchId) {
                throw new ForbiddenException('You can only respond to complaints in your branch');
            }
        }

        const complaint = await this.complaintService.addAdminResponse(id, dto, adminId);
        return {
    ...
    async resolve(
        ...
        if (userRole === UserRole.BRANCH_MANAGER) {
            const complaint = await this.complaintService.findById(id);
            if (complaint.branchId?.toString() !== userBranchId) {
                throw new ForbiddenException('You can only resolve complaints in your branch');
            }
        }
    ...
    async addFeedback(
        ...
        const userId = req.user.userId || req.user.id || req.user.sub;
        const complaint = await this.complaintService.addClientFeedback(id, dto, userId);
        return {
