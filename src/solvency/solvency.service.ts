...
export class SolvencyService {
  ...
  private extractName(text: string): string | undefined {
    ...
    const patterns = [
      ...
      /(?:full\s*name)\s*[:\-]?\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s'\-]{2,80}?)(?=\s+(?:date\s*of\s*birth|national\s*id(?:\s*\/\s*passport)?|phone|email|current\s*address)\b)/i,
      // Template-aware: "Signature Wael Gassab Date ..."
      /(?:signature)\s*[:\-]?\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s'\-]{2,80}?)(?=\s+(?:date)\b)/i,
      // Generic field-style fallbacks.
      /(?:full\s*name|name|nom\s*complet|nom)\s*[:\-]\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s'\-]{2,80})/i,
      /(?:employee|employe|worker|applicant)\s*[:\-]\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s'\-]{2,80})/i,
    ];
    ...
    for (const line of lines) {
      ...
      const alphaRatio = line.replace(/[^A-Za-zÀ-ÿ\s'\-]/g, '').length / line.length;
