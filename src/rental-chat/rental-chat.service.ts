...
import { RentalMessage, RentalMessageDocument, MessageVisibility, UserRole } from './schemas/rental-message.schema';
...
export class RentalChatService {
    ...
    private buildParticipantsByType(rental: RentalDocument, conversationType: RentalConversationType): Types.ObjectId[] {
        const agentId = rental.agentId;
        const ownerId = rental.ownerId;
        const tenantId = rental.tenantId;

        if (!agentId) {
            throw new BadRequestException('Rental is missing agentId required for chat');
        }

        if (conversationType === RentalConversationType.AGENT_OWNER) {
            if (!ownerId) throw new BadRequestException('Rental is missing ownerId required for agent-owner chat');
            return [agentId, ownerId];
        }

        if (!tenantId) throw new BadRequestException('Rental is missing tenantId required for agent-tenant chat');
    ...
    private hasSameParticipants(
        ...
        const expectedIds = new Set(expectedParticipants.map((id) => id.toString()));
        return conversation.participants.every((participantId) => expectedIds.has(participantId.toString()));
    }

    private isParticipant(conversation: RentalConversationDocument, userId: string): boolean {
        const userObjectId = new Types.ObjectId(userId);
        return conversation.participants.some((participantId) => participantId.equals(userObjectId));
    }
    ...
    private canUserSeeMessage(message: RentalMessageDocument, userRole: UserRole | null, userId: string): boolean {
        ...
        const userObjectId = new Types.ObjectId(userId);
        if (message.visibleToUsers?.some((id) => id.equals(userObjectId))) return true;
    ...
    async getOrCreateConversation(
        ...
        const expectedParticipants = this.buildParticipantsByType(rental, conversationType);
    ...
    async getOrCreateConversationPair(rentalId: string): Promise<{
        ...
        const [agentOwner, agentTenant] = await Promise.all([
            this.getOrCreateConversation(rentalId, RentalConversationType.AGENT_OWNER),
            this.getOrCreateConversation(rentalId, RentalConversationType.AGENT_TENANT),
        ]);

        return { agentOwner, agentTenant };
    }

    async getConversationsForRental(rentalId: string, userId: string): Promise<RentalConversationDocument[]> {
        ...
        for (const conversationType of conversationTypes) {
            try {
                const conversation = await this.getOrCreateConversation(rentalId, conversationType);
    ...
    async getMessages(conversationId: string, userId: string): Promise<RentalMessageDocument[]> {
        ...
        return messages.filter((msg) => this.canUserSeeMessage(msg, userRole, userId));
    }
    ...
    async sendMessage(
        ...
        if (!senderRole) throw new ForbiddenException('Sender role not found for this rental');
