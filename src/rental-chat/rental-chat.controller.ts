import { Body, Controller, Get, Param, Post, UseGuards, Request, Query, ForbiddenException } from '@nestjs/common';
...
export class RentalChatController {
    ...
    async startConversation(
        ...
        const userConversations = await this.rentalChatService.getConversationsForRental(rentalId, userId);

        if (conversationType) {
            const conversation = userConversations.find((item) => item.conversationType === conversationType);
            if (!conversation) {
                throw new ForbiddenException('User is not a participant in this conversation type');
            }
        ...
        const agentOwner = userConversations.find((item) => item.conversationType === RentalConversationType.AGENT_OWNER);
        const agentTenant = userConversations.find((item) => item.conversationType === RentalConversationType.AGENT_TENANT);
    ...
    @UseGuards(JwtAuthGuard)
    async getRentalConversations(@Param('rentalId') rentalId: string, @Request() req: any) {
        const userId = req.user.userId;
        const conversations = await this.rentalChatService.getConversationsForRental(rentalId, userId);
        return {
            rentalId,
            conversations,
        };
