...
export class RentalChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer() server!: Server;
    private logger = new Logger('RentalChatGateway');
    private userConnections: Map<string, { userId: string; conversationIds: Set<string> }> = new Map();
    ...
    async handleJoinConversation(
        ...
        try {
            const messages = await this.rentalChatService.getMessages(conversationId, user.userId);
            client.join(room);
            user.conversationIds.add(conversationId);

            this.logger.log(`✅ User ${user.userId} joined conversation ${conversationId}`);
            client.emit('joined-conversation', { conversationId, messageCount: messages.length });
            this.server.to(room).emit('user-joined', { userId: user.userId, conversationId });
        } catch (error: any) {
    ...
    handleLeaveConversation(
        ...
        this.logger.log(`✅ User ${user.userId} left conversation ${conversationId}`);
        this.server.to(room).emit('user-left', { userId: user.userId, conversationId });
    }

    @SubscribeMessage('send-message')
    async handleSendMessage(
        @ConnectedSocket() client: Socket,
        @MessageBody() data: { conversationId: string; content: string; visibleTo?: string }
    ) {
        const user = this.userConnections.get(client.id);
        if (!user) {
            client.emit('error', 'User not authenticated');
