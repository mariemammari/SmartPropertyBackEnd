...
import { RentalDocument, RentalDocumentDocument } from './schemas/rental-document.schema';
...
export class RentalDocumentService {
     ...
     */
    async create(dto: CreateRentalDocumentDto, userId: string): Promise<RentalDocumentDocument> {
        if (!dto.rentalId || !dto.documentUrl || !dto.documentType) {
            throw new BadRequestException('rentalId, documentUrl, and documentType are required');
        }

        // Verify rental exists
        const rental = await this.rentalModel.findById(new Types.ObjectId(dto.rentalId));
        if (!rental) {
            throw new NotFoundException('Rental not found');
        }

        const visibleTo = dto.visibleToUserIds ? dto.visibleToUserIds.map(id => new Types.ObjectId(id)) : [];
    ...
    async findById(id: string, userId?: string): Promise<RentalDocumentDocument> {
        ...
        // Check visibility
        if (userId && !document.isPublic && !document.visibleTo?.includes(new Types.ObjectId(userId))) {
     ...
     */
    async update(id: string, dto: UpdateRentalDocumentDto, userId: string): Promise<RentalDocumentDocument> {
        ...
        if (dto.visibleToUserIds) {
            document.visibleTo = dto.visibleToUserIds.map(id => new Types.ObjectId(id));
        }
    ...
    async getDownloadUrl(id: string, userId?: string): Promise<{ url: string; fileName: string }> {
        ...
        return {
            url: downloadUrl,
            fileName: document.fileName || `${document.documentType}_${document._id}.pdf`,
        };
    }
}
