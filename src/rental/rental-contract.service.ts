...
import { CreateRentalContractDto, SignRentalContractDto, ContractSignerRole } from './dto/create-rental-contract.dto';
...
export class RentalContractService {
     ...
     */
    async create(dto: CreateRentalContractDto, userId: string, userRole: string): Promise<RentalContractDocument> {
     ...
     */
    async findLatestByRentalId(rentalId: string): Promise<RentalContractDocument | null> {
        return this.contractModel
     ...
     */
    async signContract(id: string, userId: string, dto?: SignRentalContractDto): Promise<RentalContractDocument> {
        if (!dto?.signerRole) {
            throw new BadRequestException('signerRole is required (tenant, owner, or agent)');
        }

        const contract = await this.contractModel.findById(new Types.ObjectId(id)).exec();
        if (!contract) {
            throw new NotFoundException('Contract not found');
        }
