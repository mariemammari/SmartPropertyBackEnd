...
import {
  ...
  RentalPaymentMethod,
  RentalPaymentStatus,
} from './schemas/rental-payment.schema';
import {
  RentalContract,
...
export class RentalService {
  ...
  constructor(
    ...
    this.forceStripeSucceededOnCreate =
      (
        this.configService.get<string>('FORCE_STRIPE_SUCCEEDED_ON_CREATE') ??
        'true'
      )
        .trim()
        .toLowerCase() !== 'false';
  ...
  async create(dto: CreateRentalDto): Promise<Rental> {
    const existingActive = await this.rentalModel
      .findOne({
  ...
  async findByTenant(tenantId: string): Promise<Rental[]> {
    ...
    for (const rental of rentals) {
      const normalizedStatus = String((rental as any).status ?? '')
        .trim()
        .toLowerCase();
      ...
      const propertyKey = String(
        (rental as any).propertyId?._id ?? (rental as any).propertyId,
      );
  ...
  /**
   * Returns all rentals for a given branch.
   */
  async findByBranch(branchId: string): Promise<Rental[]> {
    return this.rentalModel
   ...
   */
  async getActiveRentalsForAccountant(
   ...
   */
  async getAccountantRentalDetail(
    rentalId: string,
    requester: AuthenticatedUserContext,
  ): Promise<any> {
    const rental = await this.rentalModel
    ...
    const payments = await this.rentalPaymentModel
      ...
      .populate(
        'invoiceId',
        'invoiceNumber total paid status date signatureUrl pdfUrl pdfPublicId paymentBreakdown billingPeriodStart billingPeriodEnd',
      )
      .exec();

    const tracking = await this.getRentalPaymentTracking(
      rentalId,
      requester,
      18,
    );
  ...
  async getRentalPaymentTracking(
    ...
    const startMonth = this.getMonthStart(
      startFrom ?? this.getInitialBillingPeriodStart(rental),
    );
    ...
    for (let i = 0; i < safeMonthsCount; i++) {
      ...
      const monthPayments = payments.filter((payment) =>
        this.isPaymentOverlappingMonth(payment, periodStart, periodEnd),
      );
  ...
  async getActiveRentalStats(
    ...
    const byStatus = rentals.reduce(
      (acc: Record<string, number>, rental) => {
    ...
    const outstandingTotal = rentals.reduce(
      (sum, rental) => sum + Number(rental.outstandingBalance ?? 0),
      0,
    );
        ...
        businessKpis: {
          collectionRatePercent:
            paidThisMonth + pendingThisMonth > 0
              ? this.round2(
                (paidThisMonth / (paidThisMonth + pendingThisMonth)) * 100,
              )
              : 0,
          avgOutstandingPerRental:
            rentals.length > 0
    ...
    return {
      ...
      personalKpis: {
        ...
        paidRentalsCount: rentals.filter(
          (r) => Number(r.outstandingBalance ?? 0) <= 0,
        ).length,
      },
    };
  }
  ...
  async getPaymentSchedule(rentalId: string): Promise<{
    ...
    const schedule = Array.from({ length: scheduleCount }).map((_, index) => {
      ...
      const isInitialPeriod =
        index === 0 &&
        this.isSameBillingMonth(dueDate, initialBillingPeriodStart);

      return {
        dueDate,
        amount: isInitialPeriod
          ? monthlyBaseAmount + depositAmount
          : monthlyBaseAmount,
      };
    });
   ...
   */
  async createFromPropertyStatusChange(
    input: CreateRentalFromStatusChangeInput,
  ): Promise<Rental> {
    console.log(
      '🔷 [createFromPropertyStatusChange] Starting rental creation with input:',
      input,
    );

    const property = await this.propertyModel.findById(input.propertyId).exec();
    console.log(
      '🔷 [createFromPropertyStatusChange] Property found:',
      property?._id,
      'Type:',
      property?.type,
    ...
    if (property.type !== TransactionType.RENT) {
      console.log(
        '❌ [createFromPropertyStatusChange] Property type is not RENT. Type:',
        property.type,
      );
      throw new BadRequestException(
        'Rental can only be created for rent transactions',
      );
    }

    const existing = await this.rentalModel
      .findOne({
    ...
    if (existing) {
      console.log(
        '🟡 [createFromPropertyStatusChange] Rental already exists:',
        existing._id,
      );
      return existing;
    }
    ...
    const listing = input.propertyListingId
      ...
      : await this.listingModel
        .findOne({ propertyId: new Types.ObjectId(input.propertyId) })
        .sort({ createdAt: -1 })
        .exec();

    console.log(
      '🔷 [createFromPropertyStatusChange] Listing found:',
      listing?._id,
    );
    if (!listing)
      throw new BadRequestException(
        'Property listing not found for rental creation',
      );
    ...
    console.log(
      '🔷 [createFromPropertyStatusChange] Tenant ID resolved:',
      tenantId,
    );
   ...
   */
  async createFromListingStatusChange(
    listingId: string,
    input: CreateRentalFromStatusChangeInput,
  ): Promise<Rental> {
  ...
  async createOfflinePayment(
    ...
    if (
      dto.paymentMethod === RentalPaymentMethod.CHEQUE &&
      (!dto.chequeDate || !dto.bankName)
    ) {
      throw new BadRequestException(
        'chequeDate and bankName are required for cheque payments',
      );
    }
  ...
  async validateSucceededPayment(
    ...
    if (
      ...
      throw new BadRequestException(
        'Only succeeded payments can be validated from this action',
      );
    }
  ...
  private async resolveTenantId(
    ...
    if (!application) {
      throw new BadRequestException(
        'tenantId is required when no approved application exists',
      );
    }
  ...
  private async notifyPaymentSuccess(
    ...
    const agent = rental.agentId
      ? await this.userModel.findById(rental.agentId).exec()
      : null;

    const recipients = [owner?.email, tenant?.email, agent?.email].filter(
    ...
    if (!payment.invoiceId) {
      throw new InternalServerErrorException(
        `Succeeded payment ${payment._id} is missing invoiceId`,
      );
    }

    const invoice = await this.financeService.findOne(
  ...
  private async ensureInvoiceForSucceededPayment(
    ...
    const invoiceId = await this.createInvoiceForPayment(
      rental,
      payment,
      accountantId,
      rentalContractId,
  ...
  private async maybeForceStripeSuccessOnCreate(
    ...
    payment.stripeChargeId =
      payment.stripeChargeId ?? `manual-bypass-${Date.now()}`;
  ...
  async createMultiMonthPayment(
    ...
    if (!this.stripe) throw new BadRequestException('Stripe is not configured');
    if (monthsCount < 2)
      throw new BadRequestException('Multi-month payment must cover 2+ months');
    ...
    const breakdown = this.getDueBreakdownForPeriod(
      rental,
      listing,
      periodStart,
      monthsCount,
    ...
    console.log(
      `💳 [createMultiMonthPayment] Bulk payment for ${monthsCount} month(s): ${totalAmount} EUR`,
    );
    console.log(
  ...
  async createTranchedPayment(
    ...
    if (!this.stripe) throw new BadRequestException('Stripe is not configured');
    if (trancheAmount <= 0)
      throw new BadRequestException('Tranche amount must be > 0');
    if (trancheNumber < 1)
      throw new BadRequestException('Tranche number must be >= 1');
    ...
    const baseDate =
      forMonth ?? rental.nextPaymentDue ?? rental.moveInDate ?? new Date();
    const periodStart = this.getMonthStart(baseDate);
    const periodEnd = this.addMonths(periodStart, 1);
    const dueBreakdown = await this.getOutstandingBreakdownForSinglePeriod(
      rental,
    ...
    console.log(
      `💳 [createTranchedPayment] Tranche #${trancheNumber} for month starting ${periodStart.toISOString()}: ${trancheAmount} EUR`,
    );
  ...
  async getPaymentStatusForPeriod(
    ...
    const paymentsForPeriod = await this.rentalPaymentModel
      .find({
      ...
      .populate(
        'invoiceId',
        'invoiceNumber status total paid date signatureUrl pdfUrl pdfPublicId',
      )
      .exec();
    ...
    const successfulPayments = paymentsForPeriod.filter(
      (p) =>
        p.status === RentalPaymentStatus.SUCCEEDED ||
        p.status === RentalPaymentStatus.VERIFIED,
    );
    const dueBreakdown = this.getDueBreakdownForPeriod(
      rental,
      listing,
      periodStart,
      1,
    ...
    const totalPaid = successfulPayments.reduce((sum, payment) => {
      const contribution = this.getPaymentContributionForMonth(
        payment,
        periodStart,
      );
    ...
    console.log(
      `📊 [getPaymentStatusForPeriod] ${periodStart.toISOString()}: ${status} (${totalPaid}/${totalDue} EUR, ${paymentsForPeriod.length} tranches)`,
    );
   ...
   */
  private getListingDepositAmount(
    ...
    const depositMonths = Number(
      (listing as any)?.contractPolicies?.depositMonths ?? 0,
    );
    if (depositMonths > 0) {
   ...
   */
  private isDepositRequiredForPeriod(
    rental: RentalDocument,
    periodStart: Date,
  ): boolean {
  ...
  private async getOutstandingBreakdownForSinglePeriod(
    ...
    const paid = await this.getPaidBreakdownForSinglePeriod(
      rental._id.toString(),
      periodStart,
      periodEnd,
    );
    const remainingTotal = Math.max(0, due.totalDue - paid.allocatedTotal);
    const normalizedOutstanding = this.allocatePaymentAmount(
      remainingTotal,
      due,
    );
  ...
  private async getPaidBreakdownForSinglePeriod(
    ...
    const paidPayments = await this.rentalPaymentModel
      .find({
        rentalId: new Types.ObjectId(rentalId),
        billingPeriodStart: { $lt: periodEnd },
        billingPeriodEnd: { $gt: periodStart },
        status: {
          $in: [RentalPaymentStatus.SUCCEEDED, RentalPaymentStatus.VERIFIED],
        },
      })
      .exec();
  ...
  private allocatePaymentAmount(
    amount: number,
    outstanding: PaymentBreakdown,
  ): PaymentAllocation {
    let remaining = Math.max(0, Number(amount || 0));

    const depositAmount = Math.min(
      remaining,
      Math.max(0, outstanding.depositAmount),
    );
    remaining -= depositAmount;

    const agencyFeeAmount = Math.min(
      remaining,
      Math.max(0, outstanding.agencyFeeAmount),
    );
    remaining -= agencyFeeAmount;

    const rentOutstanding = Math.max(0, outstanding.rentAmount);
    const rentAmount =
      Math.min(remaining, rentOutstanding) +
      Math.max(0, remaining - rentOutstanding);
   ...
   */
  private getPaymentContributionForMonth(
    payment: RentalPaymentDocument,
    monthStart: Date,
  ): PaymentAllocation {
    ...
    const hasExplicitBreakdown =
      explicitRent > 0 || explicitAgency > 0 || explicitDeposit > 0;
    ...
    const monthlyRentShare =
      coveredMonths > 0 ? totalRent / coveredMonths : totalRent;
   ...
   */
  private isPaymentOverlappingMonth(
    payment: RentalPaymentDocument,
    monthStart: Date,
    monthEnd: Date,
  ): boolean {
    return (
      payment.billingPeriodStart < monthEnd &&
      payment.billingPeriodEnd > monthStart
    );
  }
  ...
  private getCoveredMonths(payment: RentalPaymentDocument): number {
    ...
    if (payment.isMultiMonth) {
      const diff = this.getMonthDiff(
        this.getMonthStart(payment.billingPeriodStart),
        this.getMonthStart(payment.billingPeriodEnd),
      );
  ...
  private async resolveAccountantContext(
    requester: AuthenticatedUserContext,
    ...
    if (!user || user.role !== UserRole.ACCOUNTANT) {
      throw new ForbiddenException(
        'Only accountants can perform this operation',
      );
    }
  ...
  private async assertAccountantCanAccessRental(
    rental: RentalDocument,
    requester: AuthenticatedUserContext,
  ): Promise<void> {
    const accountant = await this.resolveAccountantContext(requester);
  ...
  private async assertRentalTrackingAccess(
    rental: RentalDocument,
    requester: AuthenticatedUserContext,
  ): Promise<void> {
    const userId = String(requester?.userId ?? '');
    ...
    throw new ForbiddenException(
      'You do not have access to this rental payment tracking',
    );
  }
   ...
   */
  private async refreshRentalPaymentProgress(
    rental: RentalDocument,
    payment: RentalPaymentDocument,
  ): Promise<void> {
    rental.lastPaymentDate = new Date();
    rental.outstandingBalance = Math.max(
      0,
      (rental.outstandingBalance ?? 0) - payment.amount,
    );
  ...
  private async createInvoiceForPayment(
    ...
    try {
      ...
      console.error(
        `Failed to create invoice for payment ${payment._id}:`,
        error,
      );
      return null;
    }
   ...
   */
  private async createDefaultContractForRental(
    rental: RentalDocument,
  ): Promise<void> {
    try {
      ...
      const moveOutDate =
        rental.moveOutDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000); // Default 1 year
      ...
      console.error(
        `Failed to auto-create contract for rental ${rental._id}:`,
        error,
      );
  ...
  async sendPaymentReminders(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🔍 Found ${rentalsWithUpcomingPayments.length} rental(s) with upcoming payment(s)`,
      );
      ...
      for (const rental of rentalsWithUpcomingPayments) {
        try {
          ...
          if (!tenant?.email) {
            console.log(
              `⚠️  Skipping rental ${rental._id}: Tenant email not found`,
            );
            continue;
          }
          ...
          const daysUntilDue = Math.ceil(
            (rental.nextPaymentDue!.getTime() - today.getTime()) /
            (1000 * 60 * 60 * 24),
          );
          ...
          sentCount++;
          console.log(
            `✅ Payment reminder sent to ${tenant.email} for rental ${rental._id} (due in ${daysUntilDue} days)`,
          );
        } catch (error: unknown) {
          const msg = error instanceof Error ? error.message : String(error);
          console.error(
            `❌ Failed to send reminder for rental ${rental._id}: ${msg}`,
          );
        }
      }

      console.log(
        `📧 [RentalService] Payment reminders completed. Sent ${sentCount}/${rentalsWithUpcomingPayments.length}`,
      );
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : String(error);
