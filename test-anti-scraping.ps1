...
Write-Host "   SMART PROPERTY ANTI-SCRAPING VERIFICATION"
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    $loginBody = @{ email = "ammari.mariem@esprit.tn"; password = "123456" } | ConvertTo-Json
    ...
    if ($token) {
        ...
        $authHeaders = @{ "Authorization" = "Bearer $token"; "User-Agent" = $browserUA }
    ...
    Write-Host "  [SKIP] Auth test skipped (Credentials might not be in local DB)." -ForegroundColor Yellow
}
